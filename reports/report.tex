...
\subsection{Quản lý tiến độ công việc}

Nhóm đã lựa chọn áp dụng quy trình \textbf{Agile} (theo mô hình Scrum) để đảm bảo tính linh hoạt và khả năng ứng biến nhanh với các thay đổi trong quá trình phát triển dự án. Công cụ \textbf{Jira} được sử dụng làm nền tảng chính để quản lý và theo dõi tiến độ công việc một cách trực quan.

Cụ thể, nhóm thực hiện quản lý công việc thông qua các hoạt động sau:
\begin{itemize}
    \item \textbf{Product Backlog:} Toàn bộ các yêu cầu chức năng và phi chức năng được lưu trữ tập trung trên Jira.
    \item \textbf{Sprint Planning:} Nhóm chia dự án thành các giai đoạn nhỏ (Sprints), mỗi giai đoạn kéo dài từ 1-2 tuần để hoàn thành một số lượng công việc nhất định.
    \item \textbf{Quản lý Task:} Mỗi công việc được chia thành các \textit{Task} cụ thể gắn với từng thành viên, kèm theo thời hạn (deadline) và mức độ ưu tiên.
\end{itemize}

Tiến độ được theo dõi với các trạng thái:
\begin{itemize}
    \item \textbf{To Do:} Các công việc đã được lập kế hoạch nhưng chưa bắt đầu.
    \item \textbf{In Progress:} Các công việc đang trong quá trình thực hiện.
    \item \textbf{In Review:} Các công việc đã xong phần code và đang được kiểm thử hoặc đợi review.
    \item \textbf{Done:} Các công việc đã hoàn thành.
\end{itemize}
