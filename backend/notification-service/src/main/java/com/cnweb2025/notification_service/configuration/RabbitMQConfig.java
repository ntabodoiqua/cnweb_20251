...
/**
 * Cấu hình RabbitMQ linh hoạt hỗ trợ nhiều queue
 * Tự động tạo queue, exchange và binding dựa trên MessageType enum
 */
@Configuration
public class RabbitMQConfig {
     ...
     * Map này sẽ được sử dụng trong @RabbitListener
     */
    @Bean
    public Map<MessageType, Queue> messageTypeQueues() {
        Map<MessageType, Queue> queues = new HashMap<>();
        for (MessageType messageType : MessageType.values()) {
            Queue queue = new Queue(messageType.getQueueName(), true); // durable = true
